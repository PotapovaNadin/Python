...
# print(f'Произведение крайних пар чисел списка - {result_lst}')
...
#     max = 0
#     for i in range(n):
#         if (float(a[i]) - int(a[i])) > max:
#             max = (float(a[i]) - int(a[i]))
#     return(max)
# max_a = find_max(a, n)
...
#     min = 1
#     for i in range(n):
#         if (float(a[i]) - int(a[i])) < min:
#             min = float(a[i]) - int(a[i])
#     return(min)

# min_a = find_min(a, n)

# result = max_a - min_a
# print(f'Разница между максимальным и минимальным дробными числами списка равна: {result}')
